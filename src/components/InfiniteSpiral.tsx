...
const InfiniteSpiral = ({
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      ...
      // Skip heavy card geometry calculations and DOM writes when off-screen
  ...
  const itemClassName =
    'absolute left-1/2 top-1/2 block h-[var(--spiral-height)] w-[var(--spiral-width)] overflow-hidden rounded-[var(--spiral-radius)] border-2 border-pink-300/80 bg-white/40 backdrop-blur-sm shadow-[0_12px_32px_rgba(244,114,182,0.25)] [backface-visibility:hidden] [transform-style:preserve-3d] [will-change:transform,opacity,filter] motion-reduce:transition-none transition-shadow hover:shadow-[0_16px_40px_rgba(244,114,182,0.45)]';
        ...
        {normalizedItems.map((item, index) => {
          const content = (
            <>
              ...
              />
              {item.label && (
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#2e0e18]/85 via-[#2e0e18]/40 to-transparent px-1.5 py-1 text-center pointer-events-none">
                  <span className="block text-[10px] font-bold font-mono-telemetry text-pink-100 truncate drop-shadow">
                    {item.label}
                  </span>
                </div>
              )}
